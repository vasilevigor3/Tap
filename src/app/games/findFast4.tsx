...
export const FindFastGame = (findFastProps: FindFastProps) => {
  ...
  useEffect(() => {
    if (!currentPlayerIsReady) {
      const timer = setTimeout(() => {
        ...
        leaveRoom.mutate(
          { roomId: room.roomId, playerIds: [player?.id] },
          {
            onSuccess: () => {
  ...
  const ReadyButton: React.FC<ReadyButtonProps> = ({ gameId, player }) => {
    ...
    const handleReadyButton = async () => {
      ...
      sendReadyToPlay(
        { gameId: roomId, playerId: player.id.toString() },
        {
          onSuccess: () => {
            setCurrentPlayerIsReady(true);
  ...
  useEffect(() => {
    ...
    if (gameActive) {
      ...
      if (timerRef.current === 0) {
        // Ensure not to start the timer again if it's already running
  ...
  return (
    <div className="text-2xl font-bold text-white text-center">
      ...
      {!gameActive && currentPlayerIsReady && <div>Waiting for other players...</div>}
      {gameActive && (
        <>
